...
def test_lexical_retriever_supports_expected_doc_id_hit_check():
    question = {
        "question": "How do I connect my domain DNS?",
        "expected_doc_ids": ["doc_domain"],
    }
    retriever = LexicalRetriever(
        [
            {
    ...
    retrieved_ids = [result.id for result in retriever.search(question["question"], top_k=5)]

    assert set(question["expected_doc_ids"]) & set(retrieved_ids)
